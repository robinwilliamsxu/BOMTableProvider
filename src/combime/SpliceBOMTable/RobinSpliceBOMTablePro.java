...
import combime.BOMExampleOne.BasePlugin;
import combime.BOMExampleOne.AlphaNumComparator;
import combime.BOMExampleOne.AlphaNumComparator;
import combime.BOMExampleOne.AlphaNumComparator;
import combime.BOMExampleOne.BasePlugin;
import combime.BOMExampleOne.BasePlugin;
  ...
  implements IXWorkbookProvider<IXTableData>
{
  ...
  {
    super("Splice BOM For HG", "2016", "Splice BOM workbook Provider for HG");
  }
  ...
  private List<IXWire> getWires(IXSubAssembly subAssembly)
  {
    ...
    //方法一
                for (IXSubAssembly C_childSub : subAssembly.getChildren()) {
                 IXConnectivityObject harnessObject = C_childSub.getHarnessObject();
                    if ((harnessObject != null) && ((harnessObject instanceof IXSplice))) 
                    {
                        IXSplice splice=(IXSplice) harnessObject;
                        Set<IXAbstractPin> pins = splice.getPins();
                        for(IXAbstractPin Pin: pins)
                        {
                            Set<IXAbstractConductor> conductors = Pin.getConductors();
                            for(IXAbstractConductor conductor:conductors)
                            {
                                if(conductor instanceof IXWire)
                                {
                                    IXWire SpliceWire=(IXWire) conductor;
                                    
                                    wireList.add((IXWire)SpliceWire);
...
//            }
            ...
            //添加了多级的wire，比如挂线下有多芯线，多芯线下对应的单线也需要添加进入wires
            
            
            
//            for (IXSubAssembly C_childSub : childSub.getChildren()) {
//                IXConnectivityObject C_ChildHarnessObject = C_childSub.getHarnessObject();
//                if ((C_ChildHarnessObject != null) && ((C_ChildHarnessObject instanceof IXWire))) 
//                {
//                    if ((harnessObject != null) && ((harnessObject instanceof IXSplice))) 
//                    {
//                        IXSplice splice=(IXSplice) harnessObject;
...
//                                {
//                                    IXWire SpliceWire=(IXWire) conductor;
//                                    if(SpliceWire.getAttribute("Name").equalsIgnoreCase(C_ChildHarnessObject.getAttribute("Name")))
//                                    {
//                                    wireList.add((IXWire)C_ChildHarnessObject);
//                                    }
//                                }
//                            }
//                        
    ...
    public String getWireName()
    {
        for (IXSubAssembly child : subAssembly.getChildren()) {              
            for (IXSubAssembly childSub : child.getChildren()) {
                        IXConnectivityObject ChildHarnessObject = childSub.getHarnessObject();
                        if ((ChildHarnessObject != null) && ((ChildHarnessObject instanceof IXWire))) {
                            IXWire ChildWire= (IXWire)ChildHarnessObject;
                            if(wire.getAttribute("Name").equalsIgnoreCase(ChildWire.getAttribute("Name")))
                                    {
                                          return this.wire.getAttribute("Name");
                                    }
                          }  
                        else if ((ChildHarnessObject != null) && ((ChildHarnessObject instanceof IXMulticore))) 
                        {
                            
                            IXMulticore multicore= (IXMulticore)ChildHarnessObject;
                            Set<IXAbstractConductor> conductors = multicore.getConductors();
                            for(IXAbstractConductor conductor:conductors)
                            {
                                IXWire wire=(IXWire)conductor;
                                if(wire.getAttribute("Name").equalsIgnoreCase(this.wire.getAttribute("Name")))
...
//                    for (IXSubAssembly C_childSub : childSub.getChildren()) {
//                        IXConnectivityObject C_ChildHarnessObject = C_childSub.getHarnessObject();
//                        if ((C_ChildHarnessObject != null) && ((C_ChildHarnessObject instanceof IXMulticore))) 
//                        {
//                            IXMulticore multicore= (IXMulticore)C_ChildHarnessObject;
//                            return multicore.getAttribute("Name");
//                        }
//
//                    }   
             }
    ...
    public String getWirePINname()
    {
        Set<IXWireEnd> wireends=this.wire.getWireEnds();
         String pinName="No",SplicePin="No";
     ...
     for (IXSubAssembly child : subAssembly.getChildren()) 
        {
         ...
         for (IXSubAssembly childSub : child.getChildren()) {
                 ...
                 //如果是先绞线再挂线，那么就需要显示挂线的看板号正对355的先切线后挂线的情况
                 for (IXSubAssembly C_childSub : childSub.getChildren()) {
                     IXConnectivityObject C_childharnessObject = C_childSub.getHarnessObject();
                     if ((C_childharnessObject != null) && ((C_childharnessObject instanceof IXWire))) {
                            IXWire wire= (IXWire)C_childharnessObject;
                            if(this.wire.getAttribute("Name").equals(wire.getAttribute("Name")))
                            {
                                WireKANBAN=child.getAttribute("PartNumber");
                            }
